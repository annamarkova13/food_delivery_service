with wt as (SELECT user_id,
                   order_id,
                   min(time) OVER (PARTITION BY order_id) as time,
...
FROM   (SELECT ads_campaign,
               start_date,
               day_number,
               round(users_cnt::decimal / max(users_cnt) OVER(PARTITION BY ads_campaign),
                     2) as retention
        FROM   (SELECT ads_campaign,
                       start_date,
                ...
                FROM   (SELECT user_id,
                               wt.time::date,
                               min(wt.time::date) OVER(PARTITION BY user_id) start_date,
                               ads_campaign
                        FROM   user_actions
                            LEFT JOIN wt using(user_id)) t1
                WHERE  ads_campaign != '0'
                GROUP BY ads_campaign, start_date, time::date) t2
        ORDER BY start_date, day_number) t3
